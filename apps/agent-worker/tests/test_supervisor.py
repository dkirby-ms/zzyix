...
def test_given_lease_unavailable_when_processing_then_requeues_for_recovery() -> None:
    ...
    first_run_id = control_plane.start_run(quilt_id=quilt_id, agent_principal_id=worker_id)
    control_plane.claim_lease(quilt_id=quilt_id, run_id=first_run_id, owner_principal_id=worker_id, ttl_seconds=60)
...
def test_given_active_lease_when_second_worker_claims_same_quilt_then_claim_is_rejected() -> None:
    ...
    control_plane.assign_quilt(quilt_id, worker_id)
    first_run_id = control_plane.start_run(quilt_id=quilt_id, agent_principal_id=worker_id)
    second_run_id = control_plane.start_run(quilt_id=quilt_id, agent_principal_id=worker_id)
