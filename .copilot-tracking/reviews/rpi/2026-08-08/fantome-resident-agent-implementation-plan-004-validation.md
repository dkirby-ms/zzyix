...
This is a required Phase 4 success criterion, not merely an environmental convenience, and it requires a platform or identity owner to complete. The
planning log records the same unresolved dependency as DR-01, WI-01, WI-06, and WI-09 at [fantome-resident-agent-implementation-log.md](../../../../.copilot-tracking/plans/logs/2026-08-07/fantome-resident-agent-implementation-log.md#L4-L24).
...
Phase 4 coverage is **partial for deployment readiness** and **partial for code
...
#### P4-001: HUMAN INTERVENTION REQUIRED: The worker identity has no protected API app role or Foundry RBAC
...
environmental convenience, and it requires a platform or identity owner to
complete. The planning log records the same unresolved
...
* Worker image and imperative ACA deployment: covered structurally through the
  intended CD deployment model.
...
activated in a shared environment until the human-owned P4-001 blocker and the
remaining deployment evidence gaps are resolved.

## Recommended Next Validations

* Human intervention: assign `agent.runtime` to the deployed worker identity
  and capture the assignment/resource IDs.
...
* Complete `docker build -f apps/agent-worker/Dockerfile apps/agent-worker` and
  capture the CD deployment evidence for the worker image and identity.
